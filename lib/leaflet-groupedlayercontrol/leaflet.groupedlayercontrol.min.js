...
L.Control.GroupedLayers = L.Control.extend({
    ...
    initialize: function (baseLayers, groupedOverlays, options, url) {
        var i, j;
    ...
    _update: function () {
        ...
        for (var i = 0; i < this._layers.length; i++) {
            obj = this._layers[i];
            this._addItem(obj, this._pluginUrl);
    ...
    _addItem: function (obj, pluginUrl) {
        var poiGroupNames = {
            'ru': {
              'Отель': 'hotel',
              'Хостел': 'hostel',
              'Агроусадьба': 'farmstead',
              'Кемпинг': 'camping',
              'Кемпинг (платный)':'camping-paid',
              'Питьевая вода': 'water',
              'Стол': 'table',
              'Стол с навесом': 'table-shelter',
              'Велоремонт': 'bike-repair',
              'Место для палатки': 'campsite',
              'Место отдыха': 'relax',
              'Аренда велосипедов': 'bike-rental',
              'Место для огня': 'fireplace',
              'Пляж': 'beach',
              'Кафе, ресторан': 'cafe',
              'Туалет': 'toilet',
              'Магазин': 'shop',
              'Инфопункт': 'info',
              'Станция ж/д': 'railway-station',
              'Обзорная точка': 'viewpoint',
        ...
        if (obj.group.name === layerNames.en.poi && obj.name) {
            if (poiGroupNames.en[obj.name]) {
                var icon = document.createElement('img');
                icon.setAttribute('src', pluginUrl + '/images/icons/' + poiGroupNames.en[obj.name] + '-transparent-24.png');
                label.appendChild(icon);
                label.classList.add('label-with-icon');
            } else {
                label.classList.add('label-without-icon');
            }
        }
...
L.control.groupedLayers = function (baseLayers, groupedOverlays, options, url) {
    return new L.Control.GroupedLayers(baseLayers, groupedOverlays, options, url);
};
